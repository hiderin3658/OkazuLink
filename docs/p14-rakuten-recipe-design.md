...
既存 `recipe_source` enum は `('ai_generated', 'external')` の 2 値。
将来 cookpad 等の追加に備え、enum は拡張せず **`external_provider` カラムでサブ識別** する方式を採る。

追加カラム（マイグレーション `20260504000001_p14_rakuten_recipes.sql`）:

```sql
ALTER TABLE public.recipes
  ADD COLUMN external_provider text,       -- 'rakuten' / 将来の 'cookpad' 等
  ADD COLUMN external_id bigint,           -- 各 provider の recipeId
  ADD COLUMN external_url text,            -- レシピページ URL
  ADD COLUMN external_image_url text,      -- サムネイル URL
  ADD COLUMN external_meta jsonb;          -- nickname, recipePublishday 等

-- source='external' のときは provider/id 必須を保証
ALTER TABLE public.recipes
...
-- 同じ外部レシピを 2 回 INSERT しない
CREATE UNIQUE INDEX recipes_external_unique
  ON public.recipes(external_provider, external_id)
  WHERE source = 'external';
```

`source = 'external'` かつ `external_provider = 'rakuten'` のレコードでは:
- `generated_prompt_hash` は NULL（カテゴリ単位の別キャッシュで管理）
- `steps` は `'[]'` 空配列（楽天は外部リンク前提、手順転載は規約上 NG）
