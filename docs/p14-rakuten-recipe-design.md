...
| # | 内容 | 推奨デフォルト |
|---|---|---|
| Q-P14-01 | お気に入り保存（saved_recipes）は楽天レシピも対象にするか？ | YES（recipes.id ベースなので拡張不要、UI のみ調整）|
| Q-P14-02 | 楽天モードでも `candidateCount` UI は出すか？（楽天は常に 4 件固定）| 出さない（楽天モード時は隠す）|
| Q-P14-03 | AI モード時の食材指定はそのまま、楽天モードは最初から無効化で OK？ | YES（複雑化を避ける）|
| Q-P14-04 | 詳細画面で楽天レシピの「材料」を表示するか？（表示は規約で許容、手順は不可）| YES（材料は AI モードと同様に見える）|
| Q-P14-05 | 楽天 API がダウンしたら AI に自動フォールバックするか？ | しない（明示エラー、ユーザーが切替判断）|
