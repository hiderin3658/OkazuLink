...
export const NAME_VARIANTS: readonly (readonly string[])[] = [
  ...
  ["ぴーまん", "ピーマン"],
  ["とうがらし", "唐辛子", "トウガラシ"],
  // ねぎ類の代表エントリ。単独 "ねぎ" / "葱" / "ネギ" もユーザー入力としてあり得るため
  // グループに含めるが、generateDictAliases は「単語完全一致」なので "たまねぎ" の
  // 部分文字列としてはマッチしない（誤検知なし）。複数 food にヒットした場合は
  // buildFoodIndex の "first wins" ルール（code 昇順）で代表が決まる。
  ["根深ねぎ", "長ねぎ", "ねぎ", "葱", "ネギ"],
  ["葉ねぎ", "万能ねぎ", "青ねぎ"],
  // きのこ
  ["しいたけ", "椎茸", "シイタケ"],
  ...
  ["うどん", "饂飩"],
  ["そば", "蕎麦"],
  // 食パンは食品成分表側で "角形食パン" 等の複合語のため、別表記で紐付ける
  ["角形食パン", "食パン", "しょくぱん"],
  ["フランスパン", "バゲット", "ふらんすぱん"],
  // === Phase 1+2 統合テスト 5.4 で発見の未マッチ表記を追加 ===
  // 豚ばら肉（generateMeatAliases で複合語が出るが、ばら肉単独表記も補完）
  // 代表名は "豚バラ"。"ばら" 単独だと薔薇等と紛らわしいため意図的に含めない
  ["豚バラ", "豚ばら", "ぶたばら", "ばら肉"],
  // 魚卵（食品成分表は "たらこ" 起点）
  ["たらこ", "明太子", "辛子明太子", "メンタイコ", "めんたいこ"],
  // 漬物（食品成分表は "白菜キムチ"）
  ["白菜キムチ", "キムチ"],
  // チーズ（食品成分表は "プロセスチーズ" 起点）
  ["プロセスチーズ", "スライスチーズ"],
];
